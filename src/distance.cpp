...
double rij2_pbc(const arma::mat &positions, const size_t i, const size_t j,
  ...
  for (auto k = size_t{0}; k < positions.n_rows; ++k) {
     ...
     */
    if (rijk > edge_length / 2.0)
      rijk -= edge_length;
    else if (rijk < -edge_length / 2.0)
      rijk += edge_length;
...
arma::vec rij_pbc(const arma::mat &positions, const size_t i, const size_t j,
  ...
  /* correct for periodic boundary conditions using nearest image convention
   */
  if (rij(0) > edge_length / 2.0)
    rij(0) -= edge_length;
  else if (rij(0) < -edge_length / 2.0)
    rij(0) += edge_length;

  if (rij(1) > edge_length / 2.0)
    rij(1) -= edge_length;
  else if (rij(1) < -edge_length / 2.0)
    rij(1) += edge_length;

  if (rij(2) > edge_length / 2.0)
    rij(2) -= edge_length;
  else if (rij(2) < -edge_length / 2.0)
    rij(2) += edge_length;
